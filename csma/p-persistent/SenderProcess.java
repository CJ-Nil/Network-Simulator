...
{ 
	...
	public Sender(int n,String s,String d,int fl,String df,NodeCounter nc)
	{
		this.totalNode = n;
		this.src = s;
		this.dest = d;
	...
	int generateRandom(int w){
		int range = w;
        int rand = (int)(Math.random() * range) + 0; 
        return rand;
	}
    private int backOffTime(int k){
    	int w  = (int)(Math.pow(2,k)-1);
    	int rand  = generateRandom(w);
    	return rand*5;
    }
    private boolean accessChannel(Channel medium)throws Throwable
	{
		int k;
		double p;
		boolean acc = false;
		while(!acc)
		{
			...
			while(k<5)
			{
				...
				{
					Counter.add();
					int currentnode = Counter.getNode();
					p = 0.7;
	...
	{ 
		...
		{ 
			...
			int f=1;
			//****core logic of p-persistent protocol********//
			while(f <= frameCount)
			{
...
public class SenderProcess
{
	...
	public static void main(String args[])
	{
		...
		for(int i=1;i<=nodeNo;i++)
		{
			...
			Sender sender = new Sender(nodeNo,src,dest,frameLength,filename,nc); //Sender(int n,String s,String d,int fl,String df)
			sender.start();
		}
	}
}
