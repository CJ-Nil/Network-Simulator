...
public class Sender extends Thread { 
	...
	int ackRecive()throws Throwable
	{
		int ack = 0;
		...
		for(byte[] frame:arr){
			...
			if(srcAdd.equals(dest) && destAdd.equals(src) && (type.equals("AK") || type.equals("NK"))){
				if(type.equals("NK"))
					ack = no*(-1);
				else
					ack = no;
				...
				if(_Ack == true) 
				{
					...
					while(j<fno){
						while(j<fno && j>=wl && j<=wr){
							...
							sleep(700);
							ak = ackRecive();	//recive acknoledgement
							if(ak>0)	//for acknoledgement recive
							{
								System.out.println("Recive Acknoledgement Frame:"+ak);
								if(ak>ackno)
									ackno = ak;
								wl=ackno;
								wr=(wl+w)-1;
							...
							if(ak<0)	//for nack recive
							{
								int index = ak*(-1);
								System.out.println("Recive Nack Frame:"+index);
								DataSendFrame = flist[index-1].getFrame();
								System.out.println("Resending Frame:"+(index));
								sendingFrame(DataSendFrame);
								sleep(1000);
							}
							j++;
